 ...
 */
public class AOTRawScriptInfo$Math extends RawScriptInfo {
    
    private List<RequirementInfo> m_requirements;
    private List<RawClassInfo> m_classes;
    
    public AOTRawScriptInfo$Math () {
        FoundationClassParser parser = new FoundationClassParser("System/Util/Math.jul");
        ainfo = new LazyAstInfo(parser, "System/Util/Math.jul", null);
        
        m_classes = new ArrayList<RawClassInfo>();
        m_classes.add(new RawClassInfo("Math", new AOTClassDeclInfo_Math (this)));
    ...
    public String getScriptFilePath() {
        return "System/Util/Math.jul";
    }
    
    // Types declared in this script //
    
        
    class AOTClassDeclInfo_Math extends LazyClassDeclInfo {

        public AOTClassDeclInfo_Math(RawScriptInfo minfo) {
            super(minfo);
            
        }

        private List<ParsedTypeName> m_parentNames = new ArrayList<ParsedTypeName>();
        private FQName m_fullName = new FQName("System.Util.Math");
